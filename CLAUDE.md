...
- **Human Task Worker (`human-task-queue`)**: A Human-in-the-Loop / Agent-in-the-Loop Activity Worker that persists pending tasks to a local lightweight JSON database.
- **MCP Server**: An interface exposing tools (`list_human_tasks`, `start_activity`, `complete_activity`) so humans (via Claude Desktop/Cursor) or external AI Agents can interact with the workflow mid-flight and resolve pending tasks.
